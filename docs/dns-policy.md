...
The DNSPolicy as a [GatewayAPI](https://gateway-api.sigs.k8s.io/) policy that uses `Direct Policy Attachment` as defined in the [policy attachment mechanism](https://gateway-api.sigs.k8s.io/v1alpha2/references/policy-attachment/) standard.
...
- [`ManagedZone`](https://github.com/Kuadrant/multicluster-gateway-controller/blob/main/config/crd/bases/kuadrant.io_managedzones.yaml): Kuadrant resource representing an Apex Zone in a dns provider.
...
`loadBalancing` is a required field in a DNSPolicy and contains the specification of how dns will be configured in order to provide balancing of load across multiple clusters. Fields included inside:
- `weighted` field describes how weighting will be applied to weighted dns records. Fields included inside:
  - `defaultWeight` arbitrary weight value that will be applied to weighted dns records by default. Integer value between 0 and 255.
...
- `geo` field enables the geo routing strategy. Fields included inside:
  - `defaultGeo` geo code to apply to geo dns records by default. Two digit ISO3166 alpha-2 country code (https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2).
...
A DNSPolicy that sets the `loadBalancing.weighted.defaultWeight` and nothing else produces a set of records grouped and weighted to produce a [Round Robin](https://en.wikipedia.org/wiki/Round-robin_DNS) routing strategy where all target clusters will have an equal chance of being returned in DNS queries.   
We can however manipulate how much traffic a cluster will get by using custom weights in the DNSPolicy and applying `custom-weight` attributes on the ManagedCluster to decide which records should get a specific weight. 
...
We can do this though by adding `geo-code` attributes on the ManagedCluster to show which county each cluster is in. The values can be any two digit ISO3166 alpha-2 country code (https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2).
