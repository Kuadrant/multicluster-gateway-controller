/*
Copyright 2022 pbrookes@redhat.com.
...
func (r *SecretReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	//only interested in ArgoCD cluster secrets
	if !v1.HasLabel(secret.ObjectMeta, ARGO_CLUSTER_LABEL) || secret.ObjectMeta.Labels[ARGO_CLUSTER_LABEL] != ARGO_CLUSTER_LABEL_VALUE {
		return ctrl.Result{}, nil
	}

	log.Log.Info("got Argo cluster secret", "secret", secret.Namespace+"/"+secret.Name, "server", string(secret.Data["server"]))

	clusterConfig := &ArgoClusterConfig{}
	...
	clusterClientConfig := &rest.Config{
		...
		TLSClientConfig: rest.TLSClientConfig{
			...
			CertData:   clusterConfig.TlsClientConfig.CertData,
			KeyData:    clusterConfig.TlsClientConfig.KeyData,
			ServerName: strings.SplitN(hostUrl.Host, ":", 2)[0],
		},
	}

	_, err = r.MCWatch.WatchCluster(clusterClientConfig)

	if err != nil {
		log.Log.Info("error occured", "error", err)
		return ctrl.Result{}, err
	}
	return ctrl.Result{}, nil
}
