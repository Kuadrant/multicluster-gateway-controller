...
func (r *GatewayReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	programmedStatus, requeue, reconcileErr := r.reconcileGateway(ctx, *previous, gateway)
	...
	// Update status
	clusters := getClustersFromAnnotations(*gateway)
...
func (r *GatewayReconciler) reconcileGateway(ctx context.Context, previous gatewayv1beta1.Gateway, gateway *gatewayv1beta1.Gateway) (metav1.ConditionStatus, bool, error) {
	log := log.FromContext(ctx)
	...
	for _, host := range allHosts {
		...
		if err != nil {
			return metav1.ConditionUnknown, false, err
		}
		if managedZone == nil {
	...
	for _, mh := range managedHosts {
		...
		if err != nil {
			...
			return metav1.ConditionUnknown, true, err
		}
		...
		if secret != nil {
			...
			syncObjectToAllGatewayClusters(updatedSecret, *gateway)
	...
	if !gatewayHasAttachedRoutes {
		...
		return metav1.ConditionUnknown, true, nil
	}

	return metav1.ConditionTrue, false, nil
}
...
// Apply sync annotations matching all clusters the gateway is being synced to
func syncObjectToAllGatewayClusters(obj metav1.Object, gateway gatewayv1beta1.Gateway) {
	...
	// add all sync annotations to match the gateway
	for annKey, annValue := range gateway.GetAnnotations() {
		if strings.HasPrefix(annKey, syncer.MCTC_SYNC_ANNOTATION_PREFIX) {
			annotations[annKey] = annValue
		}
	}
